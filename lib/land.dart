import 'dart:developer';

import 'package:expandable_widgets/expandable_widgets.dart';
import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:flutter_glow/flutter_glow.dart';

class Pixel extends StatefulWidget {
  const Pixel({Key? key}) : super(key: key);

  @override
  _PixelState createState() => _PixelState();
}

class _PixelState extends State<Pixel> {
  bool checkboxSelected = false;
  bool switchSelected = false;
  bool radioSelected = false;
  bool iconSelected = false;
  double sliderValue = 0;
  final GlobalKey<ScaffoldState> _globalKey =GlobalKey<ScaffoldState>();
  _showsnackbar(){

    var _snackbar= SnackBar(content: Text("Yeah! It`s Cool today"));
    _globalKey.currentState!.showSnackBar(_snackbar);

  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      key: _globalKey,
      backgroundColor: Colors.greenAccent,


      body: SingleChildScrollView(
        scrollDirection: Axis.vertical,
        child: Column(
          mainAxisSize: MainAxisSize.max,
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            SizedBox(height: 100,),
            Expandable(
              primaryWidget: Container(
                child: Text("HONEY"),
              ),
              secondaryWidget: Column(
                children: [
                  SizedBox(height: 50,),
                  Image.network("data:image/png;base64,"),
                  SizedBox(height: 30,),
                  Image.asset("assets/facebook.png"),
                  SizedBox(height: 20,),
                  Text("On Facebook, people daily updates their status according to their emotions and they also used to uplood   new photos on Facebook. For giving feedback, on their post, we always use comment section. For your special ones, you need stylish and long comments, which you can get from this article."),
                ],
              ),
              showArrowIcon: true,
              centralizePrimaryWidget: true,
              arrowLocation: ArrowLocation.right,
              animationDuration: Duration(seconds: 2),
              elevation: 10,
              backgroundColor: Colors.lightBlue,
              isClickable: true,
              padding: EdgeInsets.all(10.0),
            ),
            SizedBox(height: 30,),
            GlowButton(child: Text("GLOW"), onPressed: _showsnackbar,glowColor: Colors.lightBlue,),
            SizedBox(height: 30,),
            GlowCheckbox(value: checkboxSelected,
                enable: true,
                color: Colors.red,
                onChange: (bool value){setState(() {
                  checkboxSelected= !checkboxSelected;
                });}),
            SizedBox(height: 32,),
            GestureDetector(
              onTap: (){setState(() {
                iconSelected= !iconSelected;
              });},
              child: GlowIcon(
                iconSelected ? Icons.wb_cloudy : Icons.cloud_queue,
                color: Colors.red,
                size: 64,
                blurRadius: 9,
              ),
            ),
            SizedBox(height: 30,),
            GlowText("Glow Text",style: TextStyle(fontSize: 30, color: Colors.yellow),),
            SizedBox(height: 30,),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                GlowRadio<bool>(
                  value: true,
                  groupValue: radioSelected,
                  color: Colors.lightBlue,
                  onChange: (value) {
                    setState(() {
                      radioSelected = value;
                      log(value.toString());
                    });
                  },
                ),
                SizedBox(width: 30,),
                GlowRadio<bool>(
                  value: false,
                  color: Colors.green,
                  groupValue: radioSelected,
                  onChange: (value) {
                    setState(() {
                      radioSelected = value;
                      log(value.toString());
                    });
                  },
                ),
              ],
            ),
            SizedBox(height: 30,),
            GlowSwitch(
              onChanged: (value) {
                setState(() {
                  switchSelected = value;
                });
              },
              value: switchSelected,
              activeColor: Colors.blue.withOpacity(0.6),
              blurRadius: 4,
            ),
            SizedBox(height: 30,),
            GlowLineProgress(blurRadius: 12,glowColor: Colors.black,),
          ],
        ),
      ),

    );
  }
}
